...
- `site/sections.jsx` defines every section component (`Nav`, `Hero`, `PIIntro`, `ResearchPillars`, `Publications`, `People`, `News`, `Join`, `Footer`) plus local helpers, and exposes them on `window` via the `Object.assign(window, {...})` block at the end of the file. The `App` shell that composes them lives inline inside each page's HTML. **Anything a sub-page needs to reuse (e.g. `PUBS_FULL`, `PubRow`) must be added to that window assignment.**
- **Content data lives in `data/*.js` (plain JS, not JSX)**, each file assigning one array onto `window`: `publications.js` → `window.PUBS_FULL`, `people.js` → `window.PEOPLE` + `window.ALUMNI`, `news.js` → `window.NEWS`, `research.js` → `window.RESEARCH_PILLARS`. These load synchronously before `site/sections.jsx` so Babel-transpiled code can read them. **Edit the text of a paper, person, news item, or research pillar in the matching `data/*.js` file — do not touch `sections.jsx` for content changes.** The data files include a header comment documenting each entry's shape; keep those in sync when adding fields.
- Sub-pages live in their own directory as `<slug>/index.html` so GitHub Pages serves a clean URL like `/publications/`. Each sub-page loads the same four `../data/*.js` files and `../site/sections.jsx`, then composes its own `<App>` inline, typically reusing `Nav`/`Footer` plus section-specific components from `window`.
...
- The landing page's `Publications` section is a derived view — it filters `PUBS_FULL` for `selected: true` and shows those on the home page. The `/publications/` page shows the full archive. To promote or demote a paper, toggle the `selected` flag in `data/publications.js`; both pages update from the same source. `PUBS_FULL` entries carry `authors` (objects with `name`, optional `highlighted` for the PI, optional `note` surfaced as a tooltip), `links` (objects with PI-site label verbatim, e.g. `"🌍 Website"`, `"🤗 Dataset"`), `tags`, optional `selected`, and optional `detail` (relative path on jyhong.gitlab.io). The `/publications/` page's inline components (`Badge`, `AuthorList`, `AuthorInfo`, `LinkCapsule`, `PubEntry`) consume the full shape; `PubRow` in `sections.jsx` parses the parenthetical modifier out of `venue` (e.g. `"ICLR (Spotlight)"`) for the compact home-page list.
