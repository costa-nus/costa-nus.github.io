...
- `index.html` is the sole entry. It pulls React 18, ReactDOM 18, and `@babel/standalone` from unpkg (pinned versions with SRI hashes), then loads `site/sections.jsx` via `<script type="text/babel" src="site/sections.jsx">`. Babel transpiles the JSX on load.
- `site/sections.jsx` defines every section component (`Nav`, `Hero`, `PIIntro`, `ResearchPillars`, `Publications`, `People`, `News`, `Join`, `Footer`) plus local helpers. The `App` shell that composes them lives inline inside `index.html`. Page content (publications, people list, news items) is hard-coded as arrays near the top of each section — edit those to update the site.
