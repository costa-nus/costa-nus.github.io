...
- **News renders inline inside `site/pi-intro.jsx`, not a standalone section.** There is no News section component or `/news/` page on the live site: `PIIntro` reads `window.NEWS` (from `data/news.js`) and renders the list itself, showing the first `NEWS_PREVIEW` (7) entries with a "show more" toggle. The pre-migration standalone section (commit `d67a303`, "Migrate news") was retired and archived to `trash/news.jsx` — it is loaded by no page, so don't resurrect it expecting changes to appear. Each news entry colors its tag chips via the `NEWS_TAG_COLORS` map at the top of `pi-intro.jsx` (unknown tags fall back to ink). News `body` strings support inline Markdown — `**bold**`, `*italic*`, and `[text](href)` links — via the `mdInline` helper exposed from `site/_shared.jsx` (also used by `research.jsx`).
...
- **Publication metadata source of truth**: the canonical per-paper metadata (title, authors, date, links) lives in the PI's Hugo site, checked out as a sibling of this repo at `../jyhong.gitlab.io/content/publication/<slug>/index.md` (or `<slug>.md` for older entries). The `detail` field on each `PUBS_FULL` entry is the URL slug, which maps 1:1 to that directory/file name. The Hugo files use both YAML (`---` fence) and TOML (`+++` fence) frontmatter. Prefer the `publishDate` field (public-facing date, e.g. conference date) over `date` (often the file's creation timestamp), and handle both `:` and `=` separators when parsing. The `sync-publications` skill in `.claude/skills/` refreshes `date`, `year`, and `venue` on each `data/publications.js` entry from `publishDate` (fallback `date`) and `publication_short` in the matching Hugo file.
...
- `trash/` holds retired files kept as backups (e.g. the pre-migration `news.jsx`). Nothing here is loaded by the site — it is a recycle bin, not a source directory.
